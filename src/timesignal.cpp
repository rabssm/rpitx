...
int main(int argc, char* argv[])
{
  ...
  bool ppmSet = false;
	float ppm = 0.0f;
  char * endptr = NULL;
        struct timespec tstart={0,0}, tend={0,0}, tim={0,0}, tim2={0,0};
	while(1)
	{
	 ...
	 for (int i = 0; i < 64; i++) {
		...
		if(!NotKill)
		{
			while(running)
			{
                                ...
                                tim.tv_nsec = 1000000000 - tstart.tv_nsec;
                                nanosleep(&tim , &tim2);
                                clock_gettime(CLOCK_REALTIME, &tstart);
                                printf("Starting tone 1 at nanosecs = %.6f\n", 1.0e-9*tstart.tv_nsec);
		                ...
		                clk->SetFrequency(1000);
                                ...
                                nanosleep(&tim , &tim2);
                                clock_gettime(CLOCK_REALTIME, &tstart);
                                printf("Starting tone 2 at nanosecs = %.6f\n", 1.0e-9*tstart.tv_nsec);
